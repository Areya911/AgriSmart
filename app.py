...
ADMIN_USERNAME = os.getenv("ADMIN_USERNAME", "arya2839@org.in")
ADMIN_PASSWORD = os.getenv("ADMIN_PASSWORD", "areya123")
...
# Only enforce Secure flag in production (not needed for local http dev)
...
def allowed_image(filename: str) -> bool:
    """Return True only if filename has a safe image extension."""
    return (
        '.' in filename and
        filename.rsplit('.', 1)[1].lower() in ALLOWED_IMAGE_EXTENSIONS
    )

def save_profile_pic_safe(file_storage, dest_dir: str) -> str | None:
    """
    Validate and re-encode a profile picture upload via PIL.
    Returns the saved filename, or None if invalid/failed.
    """
    ...
    try:
        ...
        # Reject extremely small or suspiciously large images
        w, h = img.size
        if w < 10 or h < 10 or w > 8000 or h > 8000:
            return None
        filename = secure_filename(file_storage.filename)
        # Force JPEG re-encoding to strip any embedded payloads
        base, _ = os.path.splitext(filename)
        safe_filename = f"{base}_{int(time.time()*1000)}.jpg"
        save_path = os.path.join(dest_dir, safe_filename)
...
# ---------- Output sanitization ----------
ALLOWED_BLEACH_TAGS = ['p', 'strong', 'em', 'ul', 'ol', 'li', 'br', 'span']

def sanitize_ai_output(text: str) -> str:
    """Strip potentially dangerous HTML from AI model outputs using bleach."""
    if not text:
        return ''
    if BLEACH_AVAILABLE:
        return bleach.clean(text, tags=ALLOWED_BLEACH_TAGS, strip=True)
    # Fallback: escape all HTML if bleach not available
    return html.escape(text)

@app.after_request
def add_security_and_cache_headers(response):
    """Attach security headers and static-file cache headers to every response."""
...
def ajax_chat():
    ...
    bot_text = generate_bot_reply(user_query, username, lang_code=lang_code)
    ...
    # Save chat in DB only if user is logged in
    if username:
        try:
            save_chat(username, clean_markdown(user_query), bot_text)
        except Exception:
...
def crops():
    ...
    if request.method == "POST":
        ...
        # Save to user preferences history if logged in
        if "username" in session:
            user = User.query.filter_by(username=session["username"]).first()
            if user:
                current_list = user.recent_suggestions_list()
                for c in crop_suggestions:
                    if c not in current_list:
                        current_list.insert(0, c)
                user.set_recent_suggestions(current_list[:10])
                db.session.commit()
    ...
    if request.method == "POST":
        ...
        if file:
            try:
                ...
                filename = f"soil_{int(time.time()*1000)}_{secure_filename(file.filename)}"
                img.save(UPLOAD_DIR / filename, quality=85)
            except Exception as e:
            ...
            try:
                ...
                UPLOAD_DIR = Path("static/uploads")
                UPLOAD_DIR.mkdir(parents=True, exist_ok=True)
                filename = f"soil_{int(time.time()*1000)}.jpg"
                img.save(UPLOAD_DIR / filename, quality=85)
            except Exception as e:
        ...
        if img:
            ...
            ai_advice = sanitize_ai_output(ai_advice or '')
            ...
            if "username" in session:
                ...
                if user:
                    if filename:
                        user.last_soil_img = filename
                    user.last_soil_result = predicted_soil
                    user.last_soil_date = datetime.now().strftime("%Y-%m-%d %H:%M")
                    # Also append recommended crops to recent suggestions
                    current_list = user.recent_suggestions_list()
                    for c in recommended_crops:
                        if c not in current_list:
                            current_list.insert(0, c)
                    user.set_recent_suggestions(current_list[:10])
                    db.session.commit()
    ...
    if request.method == "POST":
        ...
        if img:
            ...
            if "username" in session:
                ...
                if user:
                    if filename:
                        user.last_leaf_img = filename
                    user.last_leaf_result = disease if (disease and disease != "nil") else (status or "Healthy")
                    user.last_leaf_date = datetime.now().strftime("%Y-%m-%d %H:%M")
                    db.session.commit()
...
def chatbot():
    ...
    # Load recent chat history only if user is logged in
    history = []
    if username:
        try:
...
def asr():
    ...
    try:
        ...
        webm_to_wav(FFMPEG_EXEC, temp_path, wav_path)
    ...
    try:
        if GTTS_AVAILABLE and bot_response:
            path = create_tts_gtts(bot_response, lang_code, TTS_DIR)
...
def contact():
    if request.method == "POST":
        ...
        flash("Thank you for contacting AgriSmart! Your message has been received and our team will get back to you shortly.", "success")
...
def signup():
    if request.method == "POST":
        ...
        # Securely handle profile picture upload (validate + PIL re-encode)
        profile_pic_filename = None
        file = request.files.get("profile_pic")
        if file and file.filename:
            safe_name = save_profile_pic_safe(file, PROFILE_PICS)
            if safe_name:
                profile_pic_filename = f"profile_pics/{safe_name}"
            else:
                flash("Invalid profile picture — only JPG/PNG/GIF/WEBP under 16 MB allowed.", "warning")
...
def login():
    if request.method == "POST":
        ...
        if role == "admin":
            # Compare against env-var-backed admin credentials
            ...
            # Login as farmer: lookup by username OR email
...
def profile():
    ...
    if request.method == "POST":
        ...
        if file and file.filename:
            safe_name = save_profile_pic_safe(file, PROFILE_PICS)
            if safe_name:
                user.profile_pic = f"profile_pics/{safe_name}"
            else:
                flash("Invalid profile picture — only JPG/PNG/GIF/WEBP under 16 MB allowed.", "warning")
...
def upload_user_file():
    ...
    f.save(path)
    # Return web-relative URL — never expose the absolute filesystem path
    web_url = url_for('static', filename=f'uploads/{session["username"]}/{filename}')
