...
(() => {
  ...
  stopBtn.addEventListener('click', async () => {
    try {
      ...
      // POST to /asr (transcribes, generates reply, creates TTS, and saves to DB)
      const form = new FormData();
      form.append('audio', blob, 'speech.webm');
      form.append('preferred_lang', preferredLang.value || 'auto');
      ...
      // Extract transcript
      const transcript = cleanMarkdown(asrResult.user_text || '').trim();
      ...
      const botText = cleanMarkdown(asrResult.bot_text || '').trim();

      // Handle TTS playback
      const ttsUrl = asrResult.tts_audio_url || null;
      if (ttsUrl) {
        try {
          botAudio.src = ttsUrl;
      ...
      // Add to UI history list (prepend)
      prependHistoryItem({
        username: window.sessionUser || 'You',
        user_message: transcript,
        bot_response: botText,
        timestamp: asrResult.timestamp || new Date().toISOString()
      });
